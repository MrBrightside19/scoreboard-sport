...
<template>
  <div class="bball-roster">
    <p class="bball-roster__hint">
      Nómina de básquet: número, nombre y capitán. Los rebotes y faltas se asocian a cada jugador.
    </p>
    <div class="bball-roster__grid">
      <section v-for="team in teams" :key="team.key" class="bball-roster__team">
        <h3>{{ team.label }} — {{ team.name }}</h3>
        <div v-if="rosterFor(team.key).length" class="bball-roster__list">
          <div
            v-for="player in rosterFor(team.key)"
            :key="player.id"
            class="bball-roster__row"
          >
            <a-input
              :value="player.number"
              placeholder="#"
            ...
            />
            <a-select
              :value="player.role === 'goalkeeper' || player.role === 'assistant_captain' ? 'player' : player.role"
